...
@Component({
  selector: 'app-marcador-water-polo',
  templateUrl: './marcador-water-polo.component.html',
  styleUrls: ['./marcador-water-polo.component.css'],
  providers: [ConfirmationService, ConjuntoService, DeporteService, SimpleService],
  encapsulation: ViewEncapsulation.None
})
export class MarcadorWaterPoloComponent implements OnInit {
    ...
    if (event.DeportePeriodoId) {
      this.conjuntoService.guardarSucesoParametro(event).subscribe(res => {
        ...
        if (resp) {
          ...
          if (this.sucesoPersona.Orden === 2) {

            this.actualizarVisorFaltas();
  ...
  actualizarVisorFaltas() {

    const nombre = this.NombreC;
