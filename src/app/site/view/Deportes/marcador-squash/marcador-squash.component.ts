...
@Component({
  selector: 'app-marcador-squash',
  templateUrl: './marcador-squash.component.html',
  styleUrls: ['./marcador-squash.component.css'],
  providers: [ConfirmationService, ConjuntoService, DeporteService, SimpleService],
  encapsulation: ViewEncapsulation.None
})
export class MarcadorSquashComponent implements OnInit {
  ...
  doCaptura($event) {
    ...
    // this.marcadorDatos.Tiempo = parseInt(tiempo[1], 10) + '.' + parseInt(tiempo[2], 10);
  ...
  doOpenVisor() {
    if (this.control_Periodo()) {
      ...
      window.open(
        ...
        this.planillaId
        }/${this.eventoId}/${this.deporteId}/${this.equipoA}/${this.equipoB}/pantalla${Math.floor(Math.random() * 100) + 1}`
      );
    }
  }
  actualizarVisor() {
    const dataMarcador = {
      ...
      ScoreA: this.totalPuntosA,
      ScoreB: this.totalPuntosB,
      Mensaje: '',
      ShowMensaje: true,
      Tope: 11,
      TeamA: this.dataJugadoresA,
      TeamB: this.dataJugadoresB
    };
    this._connection.invoke(
      'ActualizarMarcador',
      dataMarcador
